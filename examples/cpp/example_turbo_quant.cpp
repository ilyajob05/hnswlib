...
int main() {
    ...
    // -----------------------------------------------------------------------
    // Step 3: Build HNSW with exact L2 distances
    // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    // Step 4: Save raw vectors for re-ranking
    // -----------------------------------------------------------------------
    std::string raw_path = "example_tq_raw.tqrv";
    ...
    // -----------------------------------------------------------------------
    // Step 5: Compress index in-place
    // -----------------------------------------------------------------------
    TurboQuantSpace tq_space(dim);
    std::cout << "Compressing index (float32 " << dim * sizeof(float)
              << "B -> TQ " << tq_space.codeSizeBytes() << "B per vector, "
              << std::fixed << std::setprecision(1)
              << static_cast<float>(dim * sizeof(float))
                 / tq_space.codeSizeBytes()
              << "x compression)..." << std::flush;
    ...
    // -----------------------------------------------------------------------
    // Step 6: Save compressed index
    // -----------------------------------------------------------------------
    std::string index_path = "example_tq_index.bin";
    std::cout << "Saving compressed index to " << index_path << "..."
              << std::flush;
    hnsw.saveIndex(index_path);
    std::cout << " done" << std::endl;

    // -----------------------------------------------------------------------
    // Step 7: TQ search (without re-ranking)
    // -----------------------------------------------------------------------
    hnsw.setEf(64);
    ...
    // -----------------------------------------------------------------------
    // Step 8: TQ search + L2 re-ranking
    //
    // Pattern: search with high ef to get a large shortlist, then re-rank
    // the shortlist using exact L2 distances from the raw vectors file.
    // -----------------------------------------------------------------------
    ...
    for (size_t q = 0; q < num_queries; ++q) {
        ...
        // Step 8a: TQ search → shortlist of size rerank_ef
        auto pq = tq_space.prepareQuery(query);
        tq_space.beginSearch(pq);
        ...
        // Step 8b: Load raw vectors for candidates
        ...
        // Step 8c: Compute exact L2 distances and re-rank
        ...
        for (size_t i = 0; i < candidate_ids.size(); ++i) {
            ...
            shortlist.push_back({d, static_cast<hnswlib::labeltype>(candidate_ids[i])});
        }
    ...
    // -----------------------------------------------------------------------
    // Step 9: Load compressed index from disk and verify
    // -----------------------------------------------------------------------
    std::cout << "Loading compressed index from disk..." << std::flush;
    TurboQuantSpace tq_space2(dim);
    ...
    std::cout << "  L2 baseline (ef=64):      recall@"
              << K << " = " << std::fixed << std::setprecision(4)
              << l2_recall << std::endl;
    std::cout << "  TQ compressed (ef=64):    recall@"
              << K << " = " << tq_recall << std::endl;
    std::cout << "  TQ + L2 rerank (ef="
              << rerank_ef << "):  recall@"
              << K << " = " << rerank_recall << std::endl;
    std::cout << "  Loaded TQ (ef=64):        recall@"
              << K << " = " << loaded_recall << std::endl;
    std::cout << std::endl;

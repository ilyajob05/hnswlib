...
computeBruteForceGT(const std::vector<std::vector<float>> &base,
  ...
  std::cout << "Computing brute-force ground truth (" << Q << " queries x " << N
            << " base)..." << std::flush;
  StopWatch sw;
...
int main(int argc, char **argv) {
  ...
  bool loaded = use_fvecs ? loadFvecs(base_path, base_data, N)
  ...
  loaded = use_fvecs ? loadFvecs(query_path, all_queries, NUM_QUERIES)
  ...
  std::vector<size_t> ef_values = {10, 16,  24,  32,  48, 64,
                                   96, 128, 200, 300, 500};
...
#pragma omp parallel for schedule(dynamic)
    ...
    std::cout << "  RSS delta:  "
              << ((rss_after > rss_before)
                      ? (rss_after - rss_before) / (1024.0 * 1024.0)
                      : 0.0)
              << " MB" << std::endl;
    std::cout << "  data_size:  " << l2space.get_data_size() << " B/vec"
              << std::endl;

    // Save and report file size
    const std::string file_l2 = "bench_bigann_l2.bin";
    hnsw_l2.saveIndex(file_l2);
    std::cout << "  Index file: " << fileSize(file_l2) / (1024.0 * 1024.0)
              << " MB" << std::endl;
    ...
    for (size_t ef : ef_values) {
      ...
      std::cout << "  " << ef << "\t" << std::fixed << std::setprecision(4)
                << recall << "\t\t" << std::setprecision(1) << us_per_query
                << std::endl;
    }
    std::cout << std::endl;
  }
  ...
  // -----------------------------------------------------------------------
  // Parts B & C for each bit width
  // -----------------------------------------------------------------------
  for (int bits_per_coord : {4, 8}) {

    // -------------------------------------------------------------------
    // Part B: TurboQuantSpace HNSW build + search
    // -------------------------------------------------------------------
    std::cout << "============================================" << std::endl;
    std::cout << "  TurboQuantSpace (" << bits_per_coord
              << " bits/coord: " << (bits_per_coord - 1)
              << "-bit SQ + 1-bit QJL)" << std::endl;
    std::cout << "============================================" << std::endl;

    {
      TurboQuantSpace tqspace(vecdim, bits_per_coord, 42, 137);

      std::cout << "  data_size:  " << tqspace.get_data_size()
                << " B/vec (compression: " << std::fixed << std::setprecision(1)
      ...
      const std::string file_tq =
          "bench_bigann_tq_b" + std::to_string(bits_per_coord) + ".bin";
      hnsw_tq.saveIndex(file_tq);
      std::cout << "  Index file: " << fileSize(file_tq) / (1024.0 * 1024.0)
                << " MB" << std::endl;
    ...
    // -------------------------------------------------------------------
    std::cout << "============================================" << std::endl;
    std::cout << "  Brute-force TQ recall (b=" << bits_per_coord
              << ", distance quality)" << std::endl;
    std::cout << "============================================" << std::endl;

    {
      ...
      std::vector<std::vector<float>> bf_base(base_data.begin(),
                                              base_data.begin() + BF_N);
      std::vector<std::vector<float>> bf_queries(queries.begin(),
                                                 queries.begin() + BF_Q);
      auto bf_gt = computeBruteForceGT(bf_base, bf_queries, K);

      std::vector<std::vector<char>> codes(
          BF_N, std::vector<char>(tqspace.codeSizeBytes()));
      ...
      float bf_tq_recall =
          static_cast<float>(total_hits) / static_cast<float>(BF_Q * K);

      std::cout << "  Brute-force TQ Recall@" << K << ": " << std::fixed
                << std::setprecision(2) << (bf_tq_recall * 100.0f) << "%"
                << std::endl;
      std::cout << "  (This isolates distance quality from graph quality)"
                << std::endl;
      std::cout << std::endl;
    }
  } // end for bits_per_coord
  ...
  for (int bits_per_coord : {4, 8}) {
    std::cout << "============================================" << std::endl;
    std::cout << "  Hybrid: L2 graph + TQ search (b=" << bits_per_coord << ")"
              << std::endl;
    std::cout << "============================================" << std::endl;
...
#pragma omp parallel for schedule(dynamic)
      ...
      TurboQuantSpace tqspace(vecdim, bits_per_coord, 42, 137);
      hnswlib::HierarchicalNSW<float> hnsw_tq(&tqspace, HY_N, M,
                                              EF_CONSTRUCTION);
      std::vector<char> buf(tqspace.codeSizeBytes());
      tqspace.encodeVector(base_data[0].data(), buf.data());
      hnsw_tq.addPoint(buf.data(), 0);
      ...
      std::vector<std::vector<float>> hy_base(base_data.begin(),
                                              base_data.begin() + HY_N);
      std::vector<std::vector<float>> hy_queries(queries.begin(),
                                                 queries.begin() + HY_Q);
      ...
      std::vector<std::vector<char>> codes(
          HY_N, std::vector<char>(tqspace.codeSizeBytes()));
      for (size_t i = 0; i < HY_N; ++i)
        tqspace.encodeVector(hy_base[i].data(), codes[i].data());

      std::cout << "  TQ(b=" << bits_per_coord << ") brute-force\t\t";
      {
        size_t total_hits = 0;
        ...
        float r = static_cast<float>(total_hits) / static_cast<float>(HY_Q * K);
  ...
  for (int bits_per_coord : {4, 8}) {
    std::cout << "============================================" << std::endl;
    std::cout << "  L2 graph + TQ data (b=" << bits_per_coord
              << ", exact build, compressed search)" << std::endl;
    std::cout << "============================================" << std::endl;

    {
      ...
      if (tq_code_size > l2_data_size) {
        std::cout << "  SKIP: TQ code (" << tq_code_size << " B) > L2 slot ("
                  << l2_data_size << " B)" << std::endl;
      } else {
...
#pragma omp parallel for schedule(dynamic)
        ...
        double build_time = sw.elapsedSeconds();
        std::cout << "  L2 build time: " << std::fixed << std::setprecision(2)
                  << build_time << " s" << std::endl;
        ...
        for (size_t i = 0; i < N; ++i) {
          ...
          char *slot =
              hnsw.getDataByInternalId(static_cast<hnswlib::tableint>(i));
  ...
  for (int bits_per_coord : {4, 8}) {
    std::cout << "============================================" << std::endl;
    std::cout << "  TQ search (b=" << bits_per_coord << ") + L2 re-ranking"
              << std::endl;
    std::cout << "============================================" << std::endl;

    {
      ...
      std::cout << "  Building TQ graph on " << N << " vectors..." << std::endl;
      hnswlib::HierarchicalNSW<float> hnsw_tq(&tqspace, N, M, EF_CONSTRUCTION);
      std::vector<char> buf(tqspace.codeSizeBytes());
      tqspace.encodeVector(base_data[0].data(), buf.data());
      hnsw_tq.addPoint(buf.data(), 0);
  ...
  std::cout << "  L2Space:          " << vecdim * sizeof(float) << " bytes"
            << std::endl;
  for (int b : {4, 8}) {
    TurboQuantSpace tqs(vecdim, b, 42, 137);
    std::cout << "  TQ (b=" << b << "):        " << tqs.codeSizeBytes()

...
static std::vector<std::vector<int>> computeBruteForceGT(
    ...
    std::cerr << "[" << tag << "] Computing brute-force GT ("
              << Q << " queries x " << N << " base)..." << std::flush;
    StopWatch sw;
...
static void emitCSV(const std::string& dataset, const std::string& method,
    ...
    std::cout << dataset << "," << method << "," << ef << ","
              << std::fixed << std::setprecision(4) << recall << ","
              << std::setprecision(1) << us_per_query;
...
static void benchmarkL2(
    const std::string& tag,
    const std::vector<std::vector<float>>& base_data,
    const std::vector<std::vector<float>>& queries,
    const std::vector<std::vector<int>>& gt,
    size_t dim, size_t M, size_t ef_construction, size_t K,
...
#pragma omp parallel for schedule(dynamic)
    ...
    size_t mem_per_vec = (rss_after > rss_before)
        ? (rss_after - rss_before) / N : 0;
    ...
    for (size_t ef : ef_values) {
        ...
        std::cerr << "[" << tag << "]   ef=" << ef
                  ...
                  << ", " << std::setprecision(1) << us_per_q << " us/q"
                  << std::endl;
    }
}
...
static void benchmarkTQ(
    ...
    const size_t N = base_data.size();
    TurboQuantSpace tqspace(dim, 4, 42, 137);
    size_t code_size = tqspace.codeSizeBytes();

    std::cerr << "[" << tag << "] TQ: encoding " << N << " vectors..."
              << std::endl;
    StopWatch sw;
...
#pragma omp parallel for schedule(dynamic)
    ...
    std::cerr << "[" << tag << "] TQ encode: " << std::fixed
              << std::setprecision(2) << encode_time << " s" << std::endl;

    // Parallel insert (symmetric distance is thread-safe when prepared_query==nullptr)
    std::cerr << "[" << tag << "] TQ: inserting into HNSW..." << std::endl;
    size_t rss_before = getCurrentRSS();
    sw.reset();
...
#pragma omp parallel for schedule(dynamic)
    ...
    std::cerr << "[" << tag << "] TQ insert: " << std::fixed
              << std::setprecision(2) << insert_time << " s (total build: "
              << build_time << " s)" << std::endl;

    // Warmup
    hnsw.setEf(10);
    measureRecallTQ(hnsw, tqspace, queries, gt, K);
    std::cerr << "[" << tag << "] TQ warmup done" << std::endl;
    ...
    for (size_t ef : ef_values) {
        ...
        double us_per_q = sw.elapsedSeconds() / queries.size() * 1e6;
        emitCSV(tag, "tq", ef, recall, us_per_q, build_time, mem_per_vec, first);
        first = false;
        std::cerr << "[" << tag << "]   ef=" << ef
                  << ": recall=" << std::fixed << std::setprecision(4) << recall
                  << ", " << std::setprecision(1) << us_per_q << " us/q"
...
static void benchmarkL2GraphTQ(
    const std::string& tag,
    const std::vector<std::vector<float>>& base_data,
    const std::vector<std::vector<float>>& queries,
    const std::vector<std::vector<int>>& gt,
    ...
    if (tq_code_size > l2_data_size) {
        std::cerr << "[" << tag << "] l2graph_tq: SKIP (TQ code "
                  << tq_code_size << " B > L2 slot " << l2_data_size << " B)"
                  << std::endl;
        return;
    }

    std::cerr << "[" << tag << "] l2graph_tq: building L2 graph ("
              << N << " vectors)..." << std::endl;
...
#pragma omp parallel for schedule(dynamic)
    ...
    std::cerr << "[" << tag << "] l2graph_tq: L2 build: " << std::fixed
              << std::setprecision(2) << l2_build_time << " s" << std::endl;

    // Replace stored data with TQ codes
    std::cerr << "[" << tag << "] l2graph_tq: replacing data with TQ codes..."
              << std::endl;
    sw.reset();
    ...
    std::cerr << "[" << tag << "] l2graph_tq: encode+replace: " << std::fixed
              << std::setprecision(2) << encode_time << " s (total: "
              << build_time << " s)" << std::endl;
    ...
    hnsw.setEf(10);
    measureRecallTQ(hnsw, tqspace, queries, gt, K);
    std::cerr << "[" << tag << "] l2graph_tq warmup done" << std::endl;
    ...
    for (size_t ef : ef_values) {
        ...
        float recall = measureRecallTQ(hnsw, tqspace, queries, gt, K);
        double us_per_q = sw.elapsedSeconds() / queries.size() * 1e6;
        emitCSV(tag, "l2graph_tq", ef, recall, us_per_q,
                build_time, mem_per_vec, first);
        first = false;
        std::cerr << "[" << tag << "]   ef=" << ef
                  << ": recall=" << std::fixed << std::setprecision(4) << recall
                  << ", " << std::setprecision(1) << us_per_q << " us/q"
...
static void benchmarkTQRerank(
    ...
    std::cerr << "[" << tag << "] tq_rerank: encoding " << N << " vectors..."
              << std::endl;
    StopWatch sw;
...
#pragma omp parallel for schedule(dynamic)
    ...
    // Parallel insert
    std::cerr << "[" << tag << "] tq_rerank: inserting into HNSW..."
              << std::endl;
    size_t rss_before = getCurrentRSS();
    sw.reset();
...
#pragma omp parallel for schedule(dynamic)
    ...
    size_t tq_mem = (rss_after > rss_before)
        ? (rss_after - rss_before) / N : 0;
    size_t mem_per_vec = tq_mem + dim * sizeof(float);

    std::cerr << "[" << tag << "] tq_rerank: build: " << std::fixed
              << std::setprecision(2) << build_time << " s" << std::endl;
    ...
    std::cerr << "[" << tag << "] tq_rerank warmup done" << std::endl;
    ...
    for (size_t ef : ef_values) {
        ...
        emitCSV(tag, "tq_rerank", ef, recall, us_per_q,
                build_time, mem_per_vec, first);
        first = false;
        std::cerr << "[" << tag << "]   ef=" << ef
                  << ": recall=" << std::fixed << std::setprecision(4) << recall
...
int main(int argc, char** argv) {
    ...
    const size_t K = 10;
    const std::vector<size_t> ef_values =
        {10, 16, 24, 32, 48, 64, 96, 128, 200, 300, 500};

    std::vector<DatasetDesc> datasets = {
        {
            "bigann_sift128",
            base_dir + "/bigann/bigann_base.bvecs",
        ...
        {
            "glove100_128",
            base_dir + "/glove/glove100_128/base.fvecs",
        ...
        {
            "glove200_256",
            base_dir + "/glove/glove200_256/base.fvecs",
            base_dir + "/glove/glove200_256/query.fvecs",
            base_dir + "/glove/glove200_256/gt.ivecs",
            false,
            0
        }
    };
    ...
    for (const auto& ds : datasets) {
        // Check if dataset exists
        {
            ...
            if (!test.good()) {
                std::cerr << "[" << ds.name << "] SKIP: cannot open "
                          << ds.base_path << std::endl;
                continue;
            }
        }

        // Load base
        std::cerr << "[" << ds.name << "] Loading base from "
                  << ds.base_path << "..." << std::endl;
        std::vector<std::vector<float>> base_data;
        bool loaded = ds.is_bvecs
            ? loadBvecs(ds.base_path, base_data)
            : loadFvecs(ds.base_path, base_data);
        if (!loaded) {
            std::cerr << "[" << ds.name << "] SKIP: failed to load base"
                      << std::endl;
            continue;
        }
        ...
        // Load queries
        std::cerr << "[" << ds.name << "] Loading queries from "
                  << ds.query_path << "..." << std::endl;
        std::vector<std::vector<float>> queries;
        loaded = ds.is_bvecs
            ? loadBvecs(ds.query_path, queries, ds.max_queries)
        ...
        std::cerr << "[" << ds.name << "] Using " << queries.size()
                  << " queries" << std::endl;
        ...
        if (!ds.gt_path.empty()) {
            ...
            if (gt_test.good()) {
                gt_test.close();
                std::cerr << "[" << ds.name << "] Loading GT from "
                          << ds.gt_path << "..." << std::endl;
                loadIvecs(ds.gt_path, gt, queries.size());
                std::cerr << "[" << ds.name << "] Loaded " << gt.size()
                          << " GT entries" << std::endl;
            }
        }
        if (gt.empty()) {
        ...
        benchmarkL2(ds.name, base_data, queries, gt,
                    dim, M, EF_CONSTRUCTION, K, ef_values);

        benchmarkTQ(ds.name, base_data, queries, gt,
                    dim, M, EF_CONSTRUCTION, K, ef_values);

        benchmarkL2GraphTQ(ds.name, base_data, queries, gt,
                           dim, M, EF_CONSTRUCTION, K, ef_values);

        benchmarkTQRerank(ds.name, base_data, queries, gt,
                          dim, M, EF_CONSTRUCTION, K, ef_values);
